...
static void array_list_merge_sort_aux(list_t *list, array_list_t *array_list, uint32_t i, uint32_t j) {
    if (i >= j)
        return;
    ...
    uint8_t *left_candidate, *right_candidate, *result_candidate;
    while (i < left_length && j < right_length) {
        left_candidate = &array_list->array[convert_index_to_bytes(list, i)];
        right_candidate = &array_list->array[convert_index_to_bytes(list, j)];
        result_candidate = &buffer[convert_index_to_bytes(list, k)];

        if (list->comparator(left_candidate, right_candidate) <= 0) {
            memcpy(result_candidate, left_candidate, list->data_size);
            i++;
        } else {
            memcpy(result_candidate, right_candidate, list->data_size);
            j++;
        }
    }
    ...
    for (k = 0; k < (j - i + 1); k++) {
        result_candidate = &buffer[convert_index_to_bytes(list, k)];
        memcpy(&array_list->array[convert_index_to_bytes(list, i + k)],
            result_candidate, list->data_size);
    }
}
...
extern int8_t array_list_merge_sort(list_t *list) {
    if (list->length < 2)
        return 0;
    array_list_t *array_list = get_concrete_data(list);
    array_list_merge_sort_aux(list, array_list, 0, list->length - 1);
    return 0;
}
